...
function get_job_for_like_custmer_name($customer_name) {
    $query ="SELECT
   j.is_active,
   c.customer_name,
   j.job_id,
...
function get_batches_for_job_id($job_id) {
//    $query = "SELECT j.job_id, p.product_id, t.product_name, b.batch_id, b.start_time, b.production_time, b.inventory_time,
//  b.end_time, b.batch_Qty, (batch_Qty* p.price_per_item) as batch_value, w.workstep_name,
//  l.name as inventory_location_name, COALESCE(js.shipping_address, 'NA') as shipping_address, b.error_message, b.error_id, shipping_job_id
//  FROM
//  job j, job_product p, product t, workstep_info w, batch b, `inventory_location` l, job_shipping_schedule js
//  WHERE
//  p.job_id = j.job_id
//  AND b.job_product_id = p.job_product_id
//  AND p.product_id = t.product_id
//  AND b.workstep = w.workstep_id
//  AND l.inventory_location_id = b.inventory_location_id
//  AND j.job_id = $job_id;";

    $query= "SELECT  distinct(b.batch_id), j.job_id, p.product_id, t.product_name, b.start_time, b.production_time, b.inventory_time,
  b.end_time, b.batch_Qty, (batch_Qty* p.price_per_item) as batch_value, w.workstep_name,
  l.name as inventory_location_name, COALESCE(js.shipping_address, 'NA') as shipping_address, b.error_message, b.error_id, shipping_job_id
from batch b
left join job_product p on p.job_product_id = b.job_product_id
left join product t on t.product_id = p.product_id
left join job j on j.job_id = p.job_id
left join workstep_info w on w.workstep_id = b.workstep
left join inventory_location l on l.inventory_location_id = b.inventory_location_id
left join job_shipping_schedule js on js.job_shipping_schedule_id = b.shipping_job_id
where p.job_id = '$job_id' order by b.batch_id ASC;";

    // $query = "select * from batch where job_id=$job_id;";
    return (getDBresult($query));
}
...
do {
    ...
    if ($cmdCounter == 0) {
        ...
        $pos = stripos($cmd, "batches_for_job_id");
        if ($pos !== false) {
            if (isset($_GET["job_id"])) {
                $job_id = $_GET["job_id"];
                $return_result = get_batches_for_job_id($job_id);
        ...
        $return_result = 0;
        // $pos = stripos($cmd, "AllBatchesForJob");
        $pos = stripos($cmd, "like_customer_name");
        if ($pos !== false) {
            if (isset($_GET["customer_name"])) {
                $customer_name = $_GET["customer_name"];
                $return_result = get_job_for_like_custmer_name($customer_name);
                header('Content-Type: application/json');
                echo json_encode($return_result);
            }
            break;
...
// }
        ...
        if ($pos !== false) {
            ...
            $result = array();
            // $pos = str
            $result[0]['name'] = 'Batches processed today';
            $result[0]['value'] = $count_today_processed_batches;
            $result[1]['name'] = 'Error batches today';
